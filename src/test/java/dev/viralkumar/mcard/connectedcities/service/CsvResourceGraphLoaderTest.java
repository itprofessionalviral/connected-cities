...
class CsvResourceGraphLoaderTest {
    ...
    void loadRoutesMissingFile() {
        CsvResourceGraphLoader routeLoader = new CsvResourceGraphLoader();
        assertThrows(IllegalArgumentException.class, () -> routeLoader.loadRoutes(new ClassPathResource("small_cities.txt"), (from, to) -> {
                }),
                "File is missing");
    }
    ...
    void loadRoutesFromInputStream() throws IOException {
        CsvResourceGraphLoader routeLoader = new CsvResourceGraphLoader();
        AtomicLong routeCount = new AtomicLong();
        Resource resource = new InputStreamResource(new ClassPathResource("test-cities.txt").getInputStream());
